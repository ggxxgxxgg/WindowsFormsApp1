...
namespace WindowsFormsApp1
{
    public partial class Form1 : Form
    {
        ...
        private void button_ReadReg_ByLen_Click(object sender, EventArgs e)
        {
            ...
            UInt16 ReadLen = Convert.ToUInt16(textBox1.Text.ToString());
            write_buffer[0] = 0x80;
            ...
            else
            {
                switch (device.Opt_b)
                {
                    case (short)Device.OPT_BLOCK.B_00_7F:
                        readAddress = 0x00;
                        break;
                    case (short)Device.OPT_BLOCK.B_80_FF:
                        readAddress = 0x80;
                        break;
                    case (short)Device.OPT_BLOCK.B_NOW:
                        return;
                }
                richTextBox1.AppendText($"从地址{readAddress}读取{ReadLen}个数据\n");
                for (int i = 0; i < ReadLen; i++)
                {
                    richTextBox1.AppendText($"地址{readAddress.ToString("X2")}:0x{read_buffer[i].ToString("X2")}");
                    if ((i + 1) % 4 == 0)
                    {
        ...
        private void button_ReadReg_Selected_Click(object sender, EventArgs e)
        {
            ...
            write_buffer[1] = 0x88;
            // 将最高位设置为1
            AddresToRead = (byte)(AddresToRead | 0x80);
            //Console.WriteLine(AddresToRead.ToString("X2"));
            write_buffer[3] = AddresToRead;
            int ret = ControlSPI.VSI_WriteReadBytes(ControlSPI.VSI_USBSPI, DevIndex, 0, write_buffer, 3, read_buffer, 1);
            if (ret != ControlSPI.ERROR.SUCCESS)
            {
            ...
            {
                richTextBox1.AppendText("\n地址0x" + textBoxReadRegAddress.Text.ToString() + ":");
        ...
        private void ReadRegAll()
        {
            ...
            write_buffer[1] = 0x88;
            write_buffer[2] = 0x80;
            int ret = ControlSPI.VSI_WriteReadBytes(ControlSPI.VSI_USBSPI, DevIndex, 0, write_buffer, 3, read_buffer, 0x1f);
            if (ret != ControlSPI.ERROR.SUCCESS)
            {
                MessageBox.Show("读寄存器 错误", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            else
            {
        ...
        private void ReadRegByAddress(int address)
        {
            ...
            write_buffer[2] = (byte)(readAddress);
            int ret = ControlSPI.VSI_WriteReadBytes(ControlSPI.VSI_USBSPI, DevIndex, 0, write_buffer, 3, read_buffer, 1);
            if (ret != ControlSPI.ERROR.SUCCESS)
            {
                MessageBox.Show("读寄存器 错误", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
